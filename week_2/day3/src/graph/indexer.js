...
const indexContent = async (driver, content, source) => {
  ...
  // Use batching + limited concurrency to reduce wall time and avoid per-chunk overhead
  const EXTRACT_BATCH_SIZE = 50;     // tune: number of chunks per extract call
  const EXTRACT_CONCURRENCY = 4;     // tune: max parallel extract calls
  ...
  for (let i = 0; i < chunks.length; i += EXTRACT_BATCH_SIZE) {
    batches.push(chunks.slice(i, i + EXTRACT_BATCH_SIZE));
  }

  const batchResults = await asyncPool(EXTRACT_CONCURRENCY, batches, async (batch) => {
    // Each call should return { entities, relationships, chunkEntities }
    return extractFromChunks(batch);
  });
  ...
  for (const res of batchResults) {
    if (!res) continue;
    if (Array.isArray(res.entities)) entities.push(...res.entities);
    if (Array.isArray(res.relationships)) relationships.push(...res.relationships);

    const ce = res.chunkEntities;
    if (!ce) continue;

    // Support Map or plain object for chunkEntities
    if (ce instanceof Map) {
      for (const [idx, names] of ce.entries()) {
        const existing = chunkEntities.get(idx) ?? [];
        chunkEntities.set(idx, Array.from(new Set(existing.concat(names))));
      }
